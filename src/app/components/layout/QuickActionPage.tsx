...
const PAGE_META: Record<QuickPage, { eyebrow: string; title: string; description: string; actions: { type: ActionType; label: string }[] }> = {
  market: { eyebrow: 'LIVE MARKET INTELLIGENCE', title: 'Market Intel', description: 'Demand, competition and exploitable pressure across the current turn.', actions: [{ type: 'expand_market', label: 'Expand market' }, { type: 'validate_market', label: 'Validate market entry' }, { type: 'marketing_campaign', label: 'Launch campaign' }] },
    ...
    <section className="quick-page" role="dialog" aria-modal="true" aria-label={meta.title}>
      ...
      {page === 'news' ? (
        ...
        <div className="quick-page-list">
          ...
          {company.products.map(product => <article key={product.id}><strong>{product.name}</strong><span>{product.category.replace('_', ' ')} · compute {product.computePoints} · throughput +{Math.max(0, (product.lastComputeMultiplier - 1) * 100).toFixed(0)}% · rival edge +{(product.computeAdvantage * 100).toFixed(0)}% · margin {(product.lastTurnMargin * 100).toFixed(0)}% · revenue {money(product.lastTurnRevenue)}{product.marketValidationStatus === 'unvalidated' ? ' · SPECULATIVE / UNVALIDATED' : product.marketValidationStatus === 'validated' ? ' · MARKET VALIDATED' : ''}{product.trendTiming === 'late' ? ' · LATE' : ''}</span></article>)}
        </div>
      ) : page === 'security' ? (
